...
int SPI_init() {
	...
	ConfigPtr = XSpi_LookupConfig(XPAR_USB_SPI_DEVICE_ID);
	if (ConfigPtr == NULL) {
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	u8 SendBuf[2] = {reg + 2, val}; // Reg + 2 because want command byte write bit (1 is on, 0 is off)
//	u8 RecvBuf[2];
//	XSpi_Transfer(XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount);
	int ret_code = XSpi_Transfer(&SpiInstance, SendBuf, NULL, 2);
	if(ret_code != 0){
		xil_printf("MAXreg_wr cooked");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	u8 SendBuf[nbytes + 1]; // Need + 1 because of command byte in beginning
	SendBuf[0] = reg + 2;
	int i;
	for(i = 0; i < nbytes; ++i){
		SendBuf[i + 1] = data[i];
	}
//	u8 SendBuf[2] = {reg + 2, val};
//	u8 RecvBuf[2];
//	XSpi_Transfer(XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount);
	int ret_code = XSpi_Transfer(&SpiInstance, SendBuf, NULL, nbytes + 1);
	if(ret_code != 0){
		xil_printf("MAXbytes_wr cooked");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE RecvBuf[2];
//	XSpi_Transfer(XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount);
	int ret_code = XSpi_Transfer(&SpiInstance, SendBuf, RecvBuf, 2);
	if(ret_code != 0){
		xil_printf("MAXreg_rd cooked");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return RecvBuf[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	BYTE SendBuf[nbytes + 1];
	SendBuf[0] = reg;
	BYTE RecvBuf[nbytes + 1];
//	XSpi_Transfer(XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount);
	int ret_code = XSpi_Transfer(&SpiInstance, SendBuf, RecvBuf, nbytes + 1);
	int i;
	for(i = 0; i < nbytes; i++){
		data[i] = RecvBuf[i+1];
	}
	if(ret_code != 0){
		xil_printf("MAXbytes_rd cooked");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
}
...
void MAX3421E_reset(void) {
	Status = XGpio_Initialize(&Gpio_rst, XPAR_USB_RESET_GPIO_DEVICE_ID);
	XGpio_SetDataDirection(&Gpio_rst, 1, 0); //configure reset, and set reset to output
	Status = XGpio_Initialize(&Gpio_int, XPAR_USB_INTERRUPT_GPIO_DEVICE_ID);
...
void MAX3421E_init(void) {
	...
	//start USB timer
	Status = XTmrCtr_Initialize(&Usb_timer, XPAR_USB_TIMER_DEVICE_ID);
	if (Status != XST_SUCCESS) {
