...
{
	XGpio_DiscreteWrite (&GpioKeycode, channel, data);
}

int main() {
    init_platform();
    XGpio_Initialize(&GpioKeycode, XPAR_USB_KEYCODE_GPIO_DEVICE_ID);
   	XGpio_SetDataDirection(&GpioKeycode, 1, 0x00000000);
   	XGpio_SetDataDirection(&GpioKeycode, 2, 0x00000000);
		...
		if (GetUsbTaskState() == USB_STATE_RUNNING) {
			if (!runningdebugflag) {
				...
				if (rcode == hrNAK) {
					...
					printHex ((buf.button << 16), 1);
					printHex (buf.button, 2);
					continue;
				}
				xil_printf("X displacement: ");
				xil_printf("%d ", (signed char) buf.Xdispl);
